...
public class PlayerController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (!isGameOver && !isGamePaused) //only update the game logic if the game isn't over or isn't paused
        {
            ...
            if (Input.GetAxisRaw("Horizontal_P1") > 0) { myRB.velocity = Vector2.Scale(myRB.velocity, Vector2.up) + Vector2.right * speed * Time.deltaTime; }
            else if (Input.GetAxisRaw("Horizontal_P1") < 0) { myRB.velocity = Vector2.Scale(myRB.velocity, Vector2.up) + Vector2.left * speed * Time.deltaTime; }
